<div class="row-fluid">
    <ul class="breadcrumb">
        <?php
            ...
            $this->Html->addCrumb('Feature Groups');
            echo $this->Html->getCrumbs(' / ');
        ?>
        
    </ul>
    <h2 class="heading">Feature Groups</h2>
</div>
...
<div class="row-fluid">
	  <div class="widget widget-padding span12">
		...
		<form action="<?php echo Configure::read('root_url');?>featureGroups/index" method="get" name="search_method">
		<div class="widget-body" style="padding-bottom:0px;">
		  ...
		  <table cellpadding="10" cellspacing="8" border="0" width="100%">
			...
			<tr>
				...
				<button class="btn" type="button" onclick="window.location.href = '<?php echo Router::url('/');?>featureGroups'">Cancel</button>
				</td>
			</tr>
...
<div class="widget-header">
  ...
  <div class="widget-buttons">
      <a href="<?php echo Router::url('/').'featureGroups/add'?>" data-title="Add Feature Groups" class="tip" ><i class="icon-plus"></i></a>
...
<div class="widget-body">
  <table id="users" class="table table-striped table-bordered dataTable">
	...
	$paginatorParams = $this->Paginator->params();
	 if(count($feature_groups) > 0){
	foreach ($feature_groups as $feature_group): 
	$index++;
	?>
      <tr>
      <td><?php echo  $index + ( ((int)$this->Paginator->counter('{:page}') - 1) * $paginatorParams['limit'] ); ?>&nbsp;</td>
      <td><?php echo h($feature_group['FeatureGroup']['name']); ?>&nbsp;</td>
	  <td><?php echo h($feature_group['Category']['name']); ?>&nbsp;</td>
      <td><?php echo h(date('Y-m-d',strtotime($feature_group['FeatureGroup']['created']))); ?>&nbsp;</td>
      <td>
          <div class="btn-group">
            ...
            <ul class="dropdown-menu pull-right">
              <li><a href="<?php echo Router::url('/').'featureGroups/edit/'.$feature_group['FeatureGroup']['id'];?>"><i class="icon-edit"></i> Edit</a></li>
            </ul>
          </div>
        </td>
      </tr>
    <?php endforeach;  }else{  ?> 
	<tr>
      <td colspan="7">No Feature group available</td></tr>
