...
class PaypalHelper extends AppHelper {
	...
	{ 
		...
		/* if (!empty($settings['system_defined'])) {
			$ecnoded_system_defined_params = base64_encode(gzdeflate(serialize($settings['system_defined']) , 9));
		...
		if (!empty($settings['m_defined'])) {
			$ecnoded_message_params = base64_encode(gzdeflate(serialize($settings['m_defined']) , 9));
			$message = substr(md5(Configure::read('Security.salt') . $ecnoded_message_params) , 5, 5);
			$settings['os2'] = $settings['m_defined']['message'];
		}
		
		
		if (!empty($settings['g_defined'])) {
			$ecnoded_message_params = base64_encode(gzdeflate(serialize($settings['g_defined']) , 9));
			$message = substr(md5(Configure::read('Security.salt') . $ecnoded_message_params) , 5, 5);
		...
		/* if (!empty($settings['paymentaction']) && $settings['paymentaction'] == 'sale') {
